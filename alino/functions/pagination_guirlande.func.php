...
$req = $db->query('SELECT id_guirland FROM guirlande');
...
$sql = "SELECT * FROM guirlande ORDER BY date_g DESC $limit";
//echo $limit;
...
if ($nbr_pages != 1) {
	if ($page_num > 1) {
		...
		for($i = $page_num - $nbrearticles_max_avant_et_apres; $i < $page_num ; $i++) { 
			if ($i > 0) {
				$pagination .='<a href="index.php?page=guirlande&&pag='.$i. '">'.$i.'</a> &nbsp;';
			}
		}
	...
	if ($page_num != $nbr_page) {
		$next = $page_num + 1;
		$pagination .='<a href="index.php?page=guirlande&&pag='.$next. '"> следующая >></a> ';
	}
}
 ?>